...
def eval_p(itr, dataset, predict_dict, logger, save_path, args, plot=False, zip=False, manual=False):
    ...
    if dataset == 'shanghaitech':
    ...
    elif dataset == 'UCF_Crime_DVS':
        label_dict_path = '{}/UCF_Crime_DVS/GT'.format(args.dataset_path) #gt-ucf-dic.pickle
        #############new code###################################
        with open(file=os.path.join(label_dict_path, 'gt-ucf-dic.pickle'), mode='rb') as f:
            frame_label_dict = pickle.load(f)
        all_predict_np = np.zeros(0)
        all_label_np = np.zeros(0)
        normal_predict_np = np.zeros(0)
        ...
        for filename, v in predict_dict.items():
            if filename.startswith('Normal'):
                frame_labels = frame_label_dict[os.path.splitext(filename)[0]]

                #测试，确保维度匹配
                repeated_v = v.repeat(16)
                repeated_labels = frame_labels[:len(repeated_v)]
                if len(repeated_v) > len(repeated_labels):
                    repeated_v = repeated_v[:len(repeated_labels)]
                all_predict_np = np.concatenate((all_predict_np, repeated_v ))
                all_label_np = np.concatenate((all_label_np, repeated_labels))
                normal_label_np = np.concatenate((normal_label_np, repeated_labels))
                normal_predict_np = np.concatenate((normal_predict_np, repeated_v))
            else:
                frame_labels = frame_label_dict[os.path.splitext(filename)[0]]
                ...
                all_predict_np = np.concatenate((all_predict_np, repeated_v))
                all_label_np = np.concatenate((all_label_np, repeated_labels))
                abnormal_label_np = np.concatenate((abnormal_label_np, repeated_labels))
                abnormal_predict_np = np.concatenate((abnormal_predict_np, repeated_v))
    ...
    abnormal_auc_score = roc_auc_score(y_true=abnormal_label_np, y_score=abnormal_predict_np)             #old
    ...
    print('Iteration: {} AUC_score_abnormal_video is {}'.format(itr, abnormal_auc_score))        #old
    ...
    return itr,all_auc_score,abnormal_auc_score,all_ano_false_alarm,normal_ano_false_alarm,abnormal_ano_false_alarm
