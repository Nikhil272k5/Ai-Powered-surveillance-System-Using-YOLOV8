...
class dataset(Dataset):
    def __init__(self, args, train=True, trainlist=None, testlist=None):
        ...
        # self.video_label_dict = self.pickle_reader(             #old code
        #     file=os.path.join(self.dataset_path, self.dataset_name, 'GT', 'video_label.pickle'))
        # self.normal_video_train, self.anomaly_video_train = self.p_n_split_dataset(self.video_label_dict, self.trainlist)     #old code
        self.normal_video_train, self.anomaly_video_train = self.p_n_split_dataset(self.trainlist)       #new code
        self.train = train
        self.t_max = args.max_seqlen



    # def data_dict_creater(self): #old code
    #     data_dict = {}
    ...
    def data_dict_creater(self):   #new code
        data_dict = {}
        for filename in os.listdir(self.feature_path):
            if filename.endswith('.npy'):
                data_dict[filename] = np.load(
                    file=os.path.join(self.feature_path, filename))
        return data_dict
    ...
    # def pickle_reader(self, file=''):                #old code
    ...
    def p_n_split_dataset(self, trainlist):          #new code
        normal_video_train = []
        anomaly_video_train = []
        for t in trainlist:
            # if 'Normal' in t:
        ...
        if self.args.larger_mem:
            ...
            if self.train:
                ...
                for a_i, n_i in zip(anomaly_indexs, normaly_indexs):
                    ...
                    # anomaly_feature = np.load(             #old code
                    #     file=os.path.join(self.feature_path, anomaly_data_video_name, 'feature.npy'))
                    # file_list_anomaly = [file for file in os.listdir(self.feature_path, anomaly_data_video_name) if file.endswith('.npy')]
                    anomaly_feature = np.load(file=os.path.join(self.feature_path, anomaly_data_video_name))
                    ...
                    # file_list_normaly = [file for file in os.listdir(self.feature_path, normaly_data_video_name) if file.endswith('.npy')]
                    normaly_feature = np.load(file=os.path.join(self.feature_path, normaly_data_video_name))
                ...
                # data_video_name = self.testlist[index].replace('\n', '').replace('Ped', 'ped')
                # self.feature = np.load(file=os.path.join(self.feature_path, data_video_name, 'feature.npy'))      #old
                data_video_name = self.testlist[index].replace('\n', '').replace('Ped', 'ped')            #new
...
class dataset_train2test(Dataset):
    ...
    #             normal_video_train.append(t.replace('\n', '').replace('Ped', 'ped'))
    #     return normal_video_train, anomaly_video_train
    def p_n_split_dataset(self, trainlist):
        normal_video_train = []
        anomaly_video_train = []
        for t in trainlist:
            # if 'Normal' in t:
    ...
    # def __getitem__(self, index):             #old code
    ...
    def __getitem__(self, index):                  #new code
        data_video_name = self.trainlist[index].replace('\n', '').replace('Ped', 'ped')
        file_list = [file for file in os.listdir(self.feature_path,data_video_name) if file.endswith('.npy')]
        self.feature = [np.load(file=os.path.join(self.feature_path, file)) for file in file_list]
